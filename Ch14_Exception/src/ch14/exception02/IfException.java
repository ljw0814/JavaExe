...
/*
 * <프로그래밍의 오류를 처리하는 방식>
 * 1. 함수의 리턴값으로 성공/실패/오류 여부를 판단한다.
 * 		성공/실패: true/false 
 * 		여러가지 case : 정수(-1, 0, 1, 2...)
 * 2. C++언어 방식: Exception등장
 * 		라이브러리로 제공되는 방식이 아니라서 개발자가 모두 직접 저으이해야함
...
=> try{
	...
	// try내에서 로직 실행중 발생한 모든 에러를 처리하는 곳
}

Java는 JDK라는 라이브러리를 제공하므로 직접 여러 모듈/클래스/메서드 설계했다.
설계 과정에서 일어날 수 있는 모든 Error에 대해서 미리 정의된 Exception을
